...
PULP_FC_EXTRA_SRCS_test = /home/xiaywang/FANN-on-MCU/build/wolfe/rt_conf.c /home/xiaywang/FANN-on-MCU/build/wolfe/rt_pad_conf.c
...
$(CONFIG_BUILD_DIR)/$(PULP_APP)/$(PULP_APP): $(test_OBJS)
	mkdir -p `dirname $@`
	$(PULP_LD) $(PULP_ARCH_LDFLAGS) -MMD -MP -o $@ $^ $(PULP_LDFLAGS_test) $(PULP_LDFLAGS)

$(CONFIG_BUILD_DIR)/test.ld: $(PULP_SDK_INSTALL)/rules/tools.mk
	plpflags genlink $(FLAGS_OPT) --output-dir=$(CONFIG_BUILD_DIR) $(apps) --config-file=$(CONFIG_BUILD_DIR)/config.json

$(CONFIG_BUILD_DIR)/test_ld:
	plpflags genlink $(FLAGS_OPT) --output-dir=$(CONFIG_BUILD_DIR) $(apps) --config-file=$(CONFIG_BUILD_DIR)/config.json
...
GEN_TARGETS += $(CONFIG_BUILD_DIR)/test.ld
GEN_TARGETS_FORCE += $(CONFIG_BUILD_DIR)/test_ld
