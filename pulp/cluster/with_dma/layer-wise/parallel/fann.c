...
fann_type *fann_run(fann_type * input)
{
  ...
  rt_dma_memcpy((int)fann_weights, (int)weights_loc_buff[buff_index_weights], num_weights*datasize, RT_DMA_DIR_EXT2LOC, 0, &id_in_weights);
  ...
  for(layer_it = 1; layer_it != NUM_LAYERS; ++layer_it) {
    ...
    num_weights = fann_neurons[last_neuron-1].last_connection - first_connection;
    ...
    for(neuron_it = 0; neuron_it < num_iter_neuron; neuron_it+=nPE) { // EACH NEURON
      ...
      if(CONNECTION_RATE >= 1) { // CONNECTION_RATE
        ...
        compute_per_layer_parallel_f32((fann_type *)weights, neuron_values[buff_index_neuron_values], num_connections, DECIMAL_POINT, nPE_tmp, neuron_sum_buffer);

        buff_index_neuron_values ^= 1;

#endif // FIXEDFANN

        weights = weights + num_connections*nPE_tmp;
